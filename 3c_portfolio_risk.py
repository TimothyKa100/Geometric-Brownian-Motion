...
def portfolio_pnl(paths: np.ndarray, weights: np.ndarray) -> np.ndarray:
    ...
    weights: dollar weights, shape (n_assets,)
    Returns: P&L array, shape (n_paths,)
    """
    S_0 = paths[:, :, 0]   # (n_paths, n_assets)
    S_T = paths[:, :, -1]  # (n_paths, n_assets)
    returns = (S_T - S_0) / S_0               # relative returns
    pnl = (returns * weights).sum(axis=1)     # weighted portfolio return
    return pnl
...
def diversification_benefit(
    ...
    for i in range(n_assets):
        w_i = np.zeros(n_assets); w_i[i] = weights[i]
        pnl_i = portfolio_pnl(paths, w_i)
...
def plot_all(S0s, mus, sigmas, corr, weights, T=1.0, n_paths=30_000):
    ...
    for i in range(n_assets):
        w_i = np.zeros(n_assets); w_i[i] = weights[i]
        pnl_i = np.sort(portfolio_pnl(paths, w_i))
...
if __name__ == "__main__":
    ...
    weights = np.array([0.4,   0.35,  0.25])   # dollar weights (sum = 1)
