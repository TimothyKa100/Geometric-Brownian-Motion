...
In GBM, the diffusion term is $\sigma S_t\,\dd W_t$. This means uncertainty scales with the current level: a higher price has larger absolute fluctuations but similar relative fluctuations. This is consistent with many financial time series where percentage moves are more stable than absolute moves.
...
GBM does not mean-revert and it assumes a simple volatility structure (constant $\sigma$ in the base model), which is often too restrictive for real markets with regime changes and volatility clustering.
...
\begin{equation}
  \E[S_t] = S_0 e^{\mu t}.
\end{equation}
Validation compares this benchmark to empirical path means from simulation. In practice, the comparison is visualized through path and terminal-distribution plots (for example, \texttt{gbm\_paths.png} and \texttt{gbm\_terminal\_hist.png}). For sufficiently fine $\Delta t$ and many paths, the gap between empirical and theoretical mean shrinks; the remaining discrepancy is primarily finite-sample Monte Carlo noise.
...
\begin{enumerate}[leftmargin=1.5em]
  ...
  \item dependence structure: autocorrelation decays approximately exponentially,
  \begin{equation*}
