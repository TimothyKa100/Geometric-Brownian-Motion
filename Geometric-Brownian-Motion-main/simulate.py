...
def run_gbm_experiment(output_dir: Path) -> None:
	...
	n_est_paths = min(50, n_paths)
	mu_estimates, sigma_estimates, nlls = [], [], []
	for i in range(n_est_paths):
		est_i = estimate_gbm_mle(paths[i], dt=dt)
		mu_estimates.append(est_i.mu)
		sigma_estimates.append(est_i.sigma)
		nlls.append(est_i.nll)

	from estimation import GBMMLE
	est = GBMMLE(
		mu=float(np.mean(mu_estimates)),
		sigma=float(np.mean(sigma_estimates)),
		nll=float(np.mean(nlls)),
	)
	print(f"  (averaged over {n_est_paths} paths; mu std={np.std(mu_estimates):.4f}, "
		f"sigma std={np.std(sigma_estimates):.4f})")
...
def run_ou_experiment(output_dir: Path) -> None:
	...
	n_est_paths = min(50, n_paths)
	theta_estimates, mu_estimates, sigma_estimates, nlls = [], [], [], []
	for i in range(n_est_paths):
		est_i = estimate_ou_mle(paths[i], dt=dt)
		theta_estimates.append(est_i.theta)
		mu_estimates.append(est_i.mu)
		sigma_estimates.append(est_i.sigma)
		nlls.append(est_i.nll)

	from estimation import OUMLE
	est = OUMLE(
		theta=float(np.mean(theta_estimates)),
		mu=float(np.mean(mu_estimates)),
		sigma=float(np.mean(sigma_estimates)),
		nll=float(np.mean(nlls)),
	)
	print(f"  (averaged over {n_est_paths} paths; theta std={np.std(theta_estimates):.4f}, "
		f"mu std={np.std(mu_estimates):.4f}, sigma std={np.std(sigma_estimates):.4f})")
